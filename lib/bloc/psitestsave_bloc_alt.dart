...
class PsiTestSaveBloc extends Bloc<PsiTestSaveEvent, PsiTestSaveState> {
  ...
  Stream<PsiTestSaveState> _mapCreatePsiTestToState(
    ...
    try {
      ...
      Future<String> requestOption() async {
        var response = await http.get('$path/$DEFAULT_IMAGE_SIZE');
        String imageId = (response.headers['picsum-id']);
        String option = '$path/id/$imageId/$DEFAULT_IMAGE_SIZE';
        // print(option);
        return option;
      }

      Future<List> requestListOfOptions() async {
        var listOfPromises = List<Future>();
        for (int i = 0; i < DEFAULT_NUM_QUESTIONS * 4; i++) {
          listOfPromises.add(requestOption());
          print('added option');
        }
        var listOfAllOptions = await Future.wait(listOfPromises);
      ...
      List listOfAllOptions = await requestListOfOptions();
      ...
      for (int i = 0; i < DEFAULT_NUM_QUESTIONS; i++) {
        ...
        int rangeBegin = ((i * 4));
        int rangeEnd = (((i + 1) * 4));
        print('range begin $rangeBegin');
        print('range end $rangeEnd');
        options = listOfAllOptions.sublist(rangeBegin, rangeEnd);
